...
import { jarima } from "../data/jarima"
import { jarimaru } from "../data/jarima"
...
const PublicServices = ({ home, payment, setInput1, setInput2, setImage, setLogoname, setServicetype, setNumber }) => {
    const mobile_operators = payment && payment.types[4]
    const utilities = home.APRIL === "Aprel" ? jarima.result.merchants : jarimaru.result.merchants
    ...
    const [formattaxtype, setFormattaxtype] = useState(false)
                                ...
                                {item.values 
                                    ...
                                    item.name === "gni"
                                        ...
                                        <Grid item sx={{ mr: "auto", ml: "auto", mb: 1 }}>
                                            ...
                                            <Select sx={{ p: 1 }}
                                                input={<BootstrapInput />}
                                                value={item.name === "areas" ? filtervalue : taxtype}
                                                onChange={(e) => {item.name === "areas" ? setFiltervalue(e.target.value) : setTaxtype(e.target.value)}}
                                                onFocus={() => {
                                                    setFormatSelect(false)
                                                    setFormattaxtype(false)}}
                                            >
                                                {item.values.map((value) => (
                                            ...
                                            {formattaxtype && <Typography sx={{ color: "#dc3545" }}>
                                                {home.INPUT_ERROR_REQUIRED}
                                            </Typography>}
                                        </Grid>
                                    :
                                    item.name === "property_id" || item.name === "taxno"
                                    ?
                                    item.visible.value.includes(taxtype) &&
                                    <Grid item sx={{ mr: "auto", ml: "auto", mb: 1 }}>
                                        <Typography>
                                        <Typography><small>{item.title}</small></Typography>
                                        </Typography>
